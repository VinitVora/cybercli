...
ZAP_STARTUP_TIMEOUT = 60  # seconds to wait for ZAP daemon to start
    ...
    try:
        ...
        zap_proc = subprocess.Popen(
            ["/Applications/ZAP.app/Contents/Java/zap.sh",
             "-daemon", "-port", str(port), "-config", "api.disablekey=true", "-Dzap.log.level=DEBUG"],
        ...
        while time.time() < deadline:
            ...
            zap_proc.kill()
            raise RuntimeError(f"ZAP failed to start within {ZAP_STARTUP_TIMEOUT}s.")
